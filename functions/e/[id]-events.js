/*  functions/e/[id].js  —  ΑΠΟΕΛ 100 Χρόνια
    ------------------------------------------------------------------
    Δίνει σε κάθε ΕΚΔΗΛΩΣΗ δική της σελίδα με δικά της Open Graph tags,
    ώστε το WhatsApp / Viber / Facebook / X να δείχνουν τη φωτογραφία
    της εκδήλωσης αντί για το έμβλημα.

        https://www.apoel100years.org/e/3
              -> preview: φωτό + τίτλος + ημερομηνία
              -> ο χρήστης πατάει -> /?event=3

    Διαβάζει τα δεδομένα από το ίδιο το index.html, οπότε ΔΕΝ χρειάζεται
    ποτέ ενημέρωση: προσθέτεις εκδήλωση και δουλεύει αυτόματα.
*/
...
export async function onRequest(context) {
  ...
  try {
    ...
    if (res.ok) {
      ...
      const list = splitArray(html, 'events');
      const e = list[Number(id)];
      if (e) {
        title   = esc(field(e, 't')) || title;
        titleEn = esc(field(e, 't_en')) || title;
        const bits   = [field(e, 'full'), field(e, 'time'), field(e, 'venue')].map(esc).filter(Boolean);
        const bitsEn = [field(e, 'full_en') || field(e, 'full'),
                        field(e, 'time_en') || field(e, 'time'),
                        field(e, 'venue_en') || field(e, 'venue')].map(esc).filter(Boolean);
        desc   = clamp(bits.join(' · ') || esc(field(e, 'p')), 200);
        descEn = clamp(bitsEn.join(' · ') || desc, 200);
        const img = field(e, 'img');
        if (img && img !== 'tba') image = `${origin}/${img}.jpg`;
      }
    }
...
<link rel="canonical" href="${origin}/e/${id}">
...
<meta property="og:url" content="${origin}/e/${id}">
